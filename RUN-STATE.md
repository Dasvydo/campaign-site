...
| T36 | Run every guard on every push | 9 | verified | 2 | **PARTIAL → fixed.** Every mechanism it claims works, tested: the gates exit non-zero, the preview-server guard fires, `prebuild` fails the build, `chromium.mjs` is right in all three branches, `npm ci` installs what the gates import. **But two different broken pages got a fully green workflow.** One line of CSS, `#price,#numbers{display:none}`, removed the entire price band and the calculator and every check passed, because the jsdom harness has no layout and the two browser gates look at the consent sheet and the worked example. `scripts/verify-visible.mjs` now asks the narrow question none of them could: does each part of the argument occupy a box a person could look at, in three locales at two widths. The verifier's exact break fails it in all three | CI, plus a gate that loads a stylesheet | `.github/workflows/verify.yml`, `scripts/verify-visible.mjs`, `scripts/chromium.mjs` |
...
| T45 | Stop the next letter carrying the last one's edits | 9 | landed | 1 | **The founder found this by using the page.** Edit the draft, send it, deal the next letter, and the reader's own typing was still there on a letter it was never written for. The body is `contentEditable`, so the browser mutates those nodes directly and React never learns; `reset()` then reconciles to an identical tree and changes nothing. It matters more than a stale string: the whole argument of that section is that each draft is built from the file, and a draft carrying the last one's edits demonstrates the opposite. Each draft has an identity now and the editable body is keyed on it, so a reset rebuilds it. Reproduced against the shipped build, fixed, and added to `verify:demo`, which fails on the old build and passes on the new one. No independent pass | a new letter that is new | `src/components/Demo.tsx`, `scripts/verify-demo.mjs` |
| T46 | Draw the founder's actual logo | 9 | landed | 3 | **I claimed twice that the mark already matched his file. It did not.** I had compared my own path data against my own assumption, not against the file. A pixel diff put the disagreement at 5.5% of the frame: the shipped mark had a solid orange stem, making a conventional D beside an L, where his has no stem at all, the counter opening to the left so the teal bar of the L reads as the D's spine. That is the whole idea of the lockup and it was missing. Rebuilt from measurements off his file: the outer bowl at r254, the counter at r147, that radius worked out from its sagitta because SVG had been silently rescaling one too small for its chord. Teal and both gradient stops sampled from the file. 0.78% differing now, which at this size is the antialiasing. **Then two more copies turned up**: the footer emboss and the draft card's watermark were hand-duplicated and still drawing the old shape, stamped over his own letter. One exported definition now, read by all three. **And a broken substitution while doing that left the mark drawing nothing at all, to a green suite of 435 checks**, so every copy is asserted to draw the same two real paths; both failure modes fail all three locales | the mark, actually his, in all three places | `src/components/Hero.tsx`, `src/components/Footer.tsx`, `src/components/Demo.tsx`, `public/favicon.svg`, `scripts/harness-page.tsx`, `scripts/verify-payload.mjs` |
