...
Last updated: 2026-09-20 (waves 1-5 verified; wave 6 verified, T22 superseded, T23 landed;
              ...
              founder, recorded below; T22 closed as superseded; wave 9 is CI and the
              founder's own review pass, 7 tasks landed and none independently seen;
              1 blocked; 32 of the 41 live tasks verified; deployed and read back off
              the live bundle)
Status: executing
...
| T36 | Run every guard on every push | 9 | landed | 2 | `.github/workflows/verify.yml`: the suite, a build, then both browser gates against a real Chromium. There had been no CI at all, so every guard held only while somebody remembered to type the command. Run 1 failed and the failure is the point: `vite preview` bound to IPv6 only, and the step guard said the server never came up rather than letting both gates skip in silence and report green. Run 2 green at `e636a49` with all 16 demonstration and all 90 layout checks in the log. Also made the gates portable: `playwright-core` was undeclared and Chromium was resolved by hard-coded path, two different ones in the same container. No independent pass | CI, and gates that run off this machine | `.github/workflows/verify.yml`, `scripts/chromium.mjs`, `package.json` |
| T37 | Cut the seven pieces the founder did not want | 9 | landed | 1 | The hero dateline, the hero deck, the note under the hero button, the slug calling the worked example canned, both notes under the accuracy block, and the typeface credit. Their CSS went with them rather than being left to rot, along with an orphaned media query one of them was holding open, and `oneEmailIn` which briefly became dead. All three locales. No independent pass | 268 lines removed, 82 added | `src/content/*.ts`, `src/components/*.tsx`, `src/styles/paper.css` |
| T38 | Fix the two lines that misled | 9 | landed | 1 | The headline said "the repeat ones drafted", which reads as if only repeats get one; it says the ones your files can answer now, with the highlight moved onto `drafted` because the full clause highlighted covered three lines and swamped the hero. And the fit check was headed "Five questions" above a screen showing three, the other two behind Continue: screenshotted to confirm before changing it, now "Three questions, then two". No independent pass | two honest lines | `src/content/*.ts` |
| T39 | Show the three things that were built but not visible | 9 | landed | 1 | "What you give" above the founding trade's four bullets was `price-sr`, so a sighted reader met four obligations with nothing saying what they were, and the founder only saw the words because copy and paste picks up hidden text. The worked example's three headings sat at body weight in body ink and read as captions on the cards below them. The closing line was set to a 20 character measure, which stacked one sentence into four lines. All three fixed and screenshotted. No independent pass | a visible heading, inked beats, one line | `src/components/Price.tsx`, `src/styles/paper.css` |
| T40 | Print the drafts in the calculator | 9 | landed | 1 | It ran from inbound mail straight to hours, so a reader could work out what they saved without ever seeing the number they were being asked to buy, or whether it fitted the pooled allowance two sections down. `draftsPerMonth` already existed and was never shown. The input stays inbound mail, deliberately: a buyer can answer that and cannot estimate their own draft count before using anything. No independent pass | a derived drafts row | `src/components/Numbers.tsx`, `src/content/*.ts` |
| T41 | Make the favicon the mark | 9 | landed | 1 | It was two outlined circles and had never been the logo. The masthead mark was checked against the founder's file first, by rendering it, and already matched exactly; the favicon was the only thing that did not. Same paths now | one mark, everywhere | `public/favicon.svg` |
| T42 | Move the measured share somewhere quieter, and guard it | 9 | landed | 1 | Cut in T37, then the founder asked for it back. It is the first of the two muted notes under the accuracy block, under the bullet it quantifies, so the pair reads "what was measured, then what has not been". Better than its old home, which put the product's main limitation in the loudest place on the page directly after its best moment. **Two guards, both proved by breaking them**: the assembled sentence is asserted on the rendered page in all three locales, and deleting the render fails `verify:payload` three times naming it; the count is pinned in the prebuild because Lithuanian's case after "is" was written for seven, and moving the rate to 0.09 fails the build naming the locale files. No independent pass | the figure, quietly, asserted | `src/content/*.ts`, `src/components/WhoFor.tsx`, `scripts/verify-offer.mjs`, `scripts/harness-page.tsx` |
