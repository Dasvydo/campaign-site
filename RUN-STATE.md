...
| T51 | The founder's second read of the whole page | 10 | landed | 1 | Thirteen changes he asked for after reading it end to end. Removed: the hero's hours line, the worked example's minutes caveat, and the thirty day guarantee, all three with their contract entries, their locale strings and their harness assertions. Shortened the headline. The closing line now carries the page's amber highlight on the clause that makes its point, figure included, because a number left outside a highlight reads as an afterthought. Room under the worked example's three headings. **The calculator opens on what the reader's own package pools**, 5,000 on Desk and 10,000 on Firm, and follows them across the band unless they have dragged it themselves; the control's ceiling is the largest allowance and `validateValue` now demands that rather than forbidding it, and the four hand-computed chains were redone on paper for the new range. **Section 03 is a filing cabinet**: three static cards showing everything at once became tabs with one folder open, real ones with arrow keys, Home and End and roving focus, the closed panels left in the DOM so a search still finds them. The founding band lost a fifth block, the opt out, which is an edge case and now sits with the terms. The calculator's closing line is inked and inline. The fit check heading is three questions and one line. The registered office is a third of the height it was. Contact address changed | the page he asked for | `src/content/*.ts`, `src/components/*.tsx`, `src/styles/paper.css`, `src/lib/*.ts`, `scripts/*` |
