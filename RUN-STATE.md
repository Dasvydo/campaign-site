...
Started: 2026-09-16
Last updated: 2026-09-19, later still (waves 1-5 verified; wave 6 verified, T22 superseded, T23
              landed; wave 7 verified by four independent agents, which refuted two claims
              outright and found five things asserted by nothing; T33 added and not yet
              independently seen; 1 blocked; deployed)
Status: executing
...
| T28 | Give the hero the hours instead of the break even cost | 7 | verified | 1 | **PARTIAL.** The hours are genuinely computed: a verifier recomputed 41 by hand and moved `heroInbound` to watch the page follow. Two things are typed, not derived, and are recorded rather than fixed: "a firm of twenty" is spelled out in three locale files with no link to `covers` (though `verify-offer` pins `covers` at 20), and the da/lt endings are correct for one count each. Those counts are pinned now, so moving them fails the build | `heroHoursBack`, pinned | `src/lib/value.ts`, `scripts/verify-offer.mjs` |
| T29 | One order for the audiences, in both places they appear | 7 | verified | 1 | **CONFIRMED, UNPROTECTED.** A verifier read both orders out of a real browser in all three locales and could not break them, and found no third enumeration anywhere. But swapping two cards in one locale passes the whole suite: nothing asserts the alignment, and `who.groups[]` has no id to pair with `desks[].id`. The fix is real and can silently drift again | who matches the demo tabs | `src/content/*.ts` |
