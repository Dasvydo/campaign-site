...
| T46 | Draw the founder's actual logo | 9 | landed | 2 | **I claimed twice that the mark already matched his file. It did not.** I had compared my own path data against my own assumption, not against the file. A pixel diff put the disagreement at 5.5% of the frame: the shipped mark had a solid orange stem, making a conventional D beside an L, where his has no stem at all, the counter opening to the left so the teal bar of the L reads as the D's spine. That is the whole idea of the lockup and it was missing. Rebuilt from measurements taken off his file rather than by eye: the outer bowl at r254, the counter at r147, that radius worked out from its sagitta because SVG had been silently rescaling one too small for its chord. Teal and both gradient stops sampled from the file too. 0.78% differing now, which at this size is the antialiasing. Favicon rebuilt from the same paths | the mark, actually his | `src/components/Hero.tsx`, `public/favicon.svg` |
